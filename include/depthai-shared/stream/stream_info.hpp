...
struct StreamInfo {
    ...
    StreamInfo() {}
    StreamInfo(const char* name_) : name(name_) {
        do_size_correction_if_needed();
    }
    StreamInfo(const char* name_, uint32_t size_) : name(name_), size(size_) {
        do_size_correction_if_needed();
    }
    StreamInfo(const char* name_, uint32_t size_, const std::vector<int>& dims) : name(name_), size(size_), dimensions(dims) {
        do_size_correction_if_needed();
    }
