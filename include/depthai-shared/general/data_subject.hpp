...
class DataSubject {
   ...
   public:
    ...
    boost::signals2::connection connect(const DataInfo& data_info, const SlotType& subscriber) {
   ...
   protected:
    void notifyObservers(const DataInfo& data_info, const DataContainer& data) {
        const std::string& data_name = data_info.getName();
        assert(_signal_map.find(data_name) != _signal_map.end());
