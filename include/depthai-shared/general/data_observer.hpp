...
class DataObserver {
   public:
    ...
    void observe(DataSubject<DataInfo, DataContainer>& subject, const DataInfo& data_info) {
        const std::string& data_name = data_info.getName();
        _connections[data_name] = subject.connect(data_info, boost::bind(&DataObserver::onNewData, this, _1, _2));

        onNewDataSubject(data_info);
    }

   protected:
    virtual void onNewDataSubject(const DataInfo& data_info) {
        (void)(data_info);
    }
