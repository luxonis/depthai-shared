...
std::unordered_map<std::string, StreamInfo> g_streams_pc_to_myriad = {{"config_h2d", StreamInfo("config_h2d", 1000)},
                                                                      {"host_capture", StreamInfo("host_capture", sizeof(CaptureMetadata))}};

std::unordered_map<std::string, StreamInfo> c_streams_myriad_to_pc = {{"left", StreamInfo("left", 921600 + sizeof(FrameMetadata), {720, 1280})},
