...
float calculateDisparityToDepthMM(unsigned disparity, unsigned width, float fov, float base_line_dist) {
    if(disparity == 0) {
        return -1.f;
    } else {
...
std::vector<uint16_t> generateLUTDisparityToDepthMM(unsigned width, float fov, float base_line_dist) {
